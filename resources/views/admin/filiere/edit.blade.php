<x-app-layout>
    ...
    <form action="{{ route("filieres.update", $filiere->id) }}" method="POST">
        ...
        <div class="w-full max-w-sm min-w-[200px]"> 
            ...
            <div class="relative">
                <select
                    ...
                    <option {{ old("annee") }} value="1">1er annee</option>
                    <option {{ old("annee") }} value="2">2eme annee</option>
                </select>
        ...
        <div class="w-full max-w-sm min-w-[200px] mt-3">
            <input name="nom" value="{{ old("nom") }}" class="w-full bg-transparent placeholder:text-slate-400 text-slate-700 text-sm border border-slate-200 rounded-md px-3 py-2 transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-300 shadow-sm focus:shadow" placeholder="Enter option filiere">
        </div>
        {{-- Option --}}
        <div class="w-full max-w-sm min-w-[200px] mt-3">
            <input name="option" value="{{ old("option") }}" class="w-full bg-transparent placeholder:text-slate-400 text-slate-700 text-sm border border-slate-200 rounded-md px-3 py-2 transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-300 shadow-sm focus:shadow" placeholder="Enter option filiere">
        </div>
