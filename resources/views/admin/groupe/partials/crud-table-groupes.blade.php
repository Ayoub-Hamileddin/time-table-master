...
<div class="mx-5 mt-5">
    <div x-data="{selectedId:null}" class="relative flex flex-col w-[1100px] mx-auto h-auto overflow-scroll text-gray-700 bg-white shadow-md rounded-lg bg-clip-border">
       <div class="w-[100%] flex justify-end my-2">
         <a href="{{ route("groupes.create") }}"  class="mr-3 py-2 px-4 text-white  shadow-md bg-blue-600 hover:bg-blue-700 rounded-lg hover:shadow-lg ">
             Add Groupe
         </a>
       </div>
        ...
        @foreach ($groupes as $groupe )
            <tr class="hover:bg-slate-50 border-b border-slate-200">
              <td class="p-4 py-5">
                <p class="block text-sm text-slate-800">{{ $groupe->id }}</p>
              </td>
              <td class="p-4 py-5">
                <p class="block text-sm text-slate-800">{{ $groupe->code }}</p>
              </td>
              <td class="p-4 py-5">
                <p class="block text-sm text-slate-800">{{ $groupe->filiere->nom }}</p>
              </td>
              <td class="p-4 py-5">
        ...
        <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
          <form method="post"  x-bind:action="'/groupes/' + selectedId" class="p-6">
              @csrf
              @method('delete')
